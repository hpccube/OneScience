...
def compute_loss(coords):
    ...
    # 应变计算
    E_xx = torch.autograd.grad(ux, coords, grad_outputs=torch.ones_like(ux), create_graph=True)[0][:, 0]
    E_yy = torch.autograd.grad(uy, coords, grad_outputs=torch.ones_like(uy), create_graph=True)[0][:, 1]
    E_xy = 0.5*(torch.autograd.grad(ux, coords, grad_outputs=torch.ones_like(ux), create_graph=True)[0][:, 1] + 
            torch.autograd.grad(uy, coords, grad_outputs=torch.ones_like(uy), create_graph=True)[0][:, 0])
    ...
    # 平衡方程残差
    Sxx_x = torch.autograd.grad(sx, coords, grad_outputs=torch.ones_like(sx), create_graph=True)[0][:, 0]
    Sxy_y = torch.autograd.grad(sxy, coords, grad_outputs=torch.ones_like(sxy), create_graph=True)[0][:, 1]
    momentum_x = Sxx_x + Sxy_y
    
    Syy_y = torch.autograd.grad(sy, coords, grad_outputs=torch.ones_like(sy), create_graph=True)[0][:, 1]
    Sxy_x = torch.autograd.grad(sxy, coords, grad_outputs=torch.ones_like(sxy), create_graph=True)[0][:, 0]
    ...
    # PDE损失（域内点）
    mask_domain = (torch.sqrt(coords[:,0]**2  + coords[:,1]**2) > 0.2) & ((coords[:,0] > 0) & (coords[:,1] > 0))   # 简单域内判断
    Equilibrium_loss = torch.mean(momentum_x[mask_domain]**2) + torch.mean(momentum_y[mask_domain]**2)
    Stress_loss = torch.mean(stress_x_res[mask_domain]**2) + torch.mean(stress_y_res[mask_domain]**2) + torch.mean(stress_xy_res[mask_domain]**2)
    # 边界条件损失
    ...
    # 圆孔边界：面力为零
    hole_radius = 0.2
    hole_mask = (torch.sqrt(coords[:,0]**2 + coords[:,1]**2) - hole_radius).abs() < 1e-4
    ...
    return Equilibrium_loss, Stress_loss, right_loss, top_loss, left_loss, bottom_loss, traction_loss
...
def train():
    ...
    for epoch in range(n_epochs):
        optimizer.zero_grad()
        Equilibrium_loss, Stress_loss, right_loss, top_loss, left_loss, bottom_loss, traction_loss = compute_loss(coords)
        total_loss = 10 * Equilibrium_loss + Stress_loss + right_loss + top_loss + left_loss + bottom_loss + traction_loss
        total_loss.backward()
        optimizer.step()
        scheduler.step()
        if epoch % 100 == 0:
            print(f'Epoch {epoch+1}/{n_epochs}, Total Loss: {total_loss.item():.6f}, Equilibrium Loss : {Equilibrium_loss.item():.6f}, Stress Loss : {Stress_loss.item():.6f}, '
    ...
    for stage in range(50):
        ...
        if last_loss_terms[0] is not None:
            ...
            print(f'L-BFGS Stage [{stage+1}/50] Step {(stage+1)*500} | '
