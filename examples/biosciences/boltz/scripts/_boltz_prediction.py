"""Boltz prediction orchestration used by the biosciences example."""
...
from onescience.datapipes.boltz.module.inference import BoltzInferenceDataModule
from onescience.datapipes.boltz.module.inferencev2 import Boltz2InferenceDataModule
from onescience.datapipes.boltz.mol import load_canonicals
from onescience.datapipes.boltz.msa.mmseqs2 import run_mmseqs2
from onescience.datapipes.boltz.parse.a3m import parse_a3m
from onescience.datapipes.boltz.parse.csv import parse_csv
from onescience.datapipes.boltz.parse.fasta import parse_fasta
from onescience.datapipes.boltz.parse.yaml import parse_yaml
from onescience.datapipes.boltz.types import MSA, Manifest, Record
from onescience.datapipes.boltz.write.writer import BoltzAffinityWriter, BoltzWriter
from onescience.models.boltz.boltz1 import Boltz1
from onescience.models.boltz.boltz2 import Boltz2
...
def require_local_asset(path: Path, description: str, directory: bool = False) -> None:
    """Require a user-provided local model or dataset asset."""
    exists = path.is_dir() if directory else path.is_file()
    if not exists:
        kind = "directory" if directory else "file"
        msg = f"Missing {description} {kind}: {path}"
...
@click.option(
    ...
    help="Directory containing the user-provided Boltz checkpoint files.",
    required=True,
)
@click.option(
    "--dataset_dir",
    type=click.Path(exists=True, file_okay=False),
    help="Directory containing the user-provided CCD and molecule data.",
    required=True,
)
@click.option(
    "--checkpoint",
...
@click.option(
    "--no_kernels/--use_kernels",
    default=True,
    help="Disable or explicitly enable the optional cuEquivariance kernels.",
)
@click.option(
    "--write_embeddings",
    is_flag=True,
...
def predict(  # noqa: C901, PLR0915, PLR0912
    ...
    # Resolve user-provided local assets. This integration never downloads assets.
    ...
    # Select and validate the local structure checkpoint and chemical reference data.
    if model == "boltz1":
        ccd_path = dataset_dir / "ccd.pkl"
        mol_dir = dataset_dir / "mols"
        require_local_asset(ccd_path, "Boltz-1 CCD")
        if checkpoint is None:
            checkpoint = model_dir / "boltz1_conf.ckpt"
    elif model == "boltz2":
        ccd_path = dataset_dir / "ccd.pkl"
        mol_dir = dataset_dir / "mols"
        require_local_asset(mol_dir, "Boltz-2 molecule reference", directory=True)
        if checkpoint is None:
            checkpoint = model_dir / "boltz2_conf.ckpt"
    else:
        raise ValueError(f"Model {model} not supported. Supported: boltz1, boltz2.")
    checkpoint = Path(checkpoint).expanduser().resolve()
    require_local_asset(checkpoint, f"{model} structure checkpoint")
    ...
    if any(r.affinity for r in manifest.records):
        ...
        if affinity_checkpoint is None:
            affinity_checkpoint = model_dir / "boltz2_aff.ckpt"
        affinity_checkpoint = Path(affinity_checkpoint).expanduser().resolve()
        require_local_asset(affinity_checkpoint, "Boltz-2 affinity checkpoint")
