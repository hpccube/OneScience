"""Boltz training orchestration used by the biosciences example.

User-facing argument parsing belongs to examples; this module only exposes the
programmatic training service used by those entrypoints.
"""
...
from onescience.datapipes.boltz.module.training import BoltzTrainingDataModule, DataConfig
...
def train(raw_config: str, args: list[str]) -> None:  # noqa: C901, PLR0912, PLR0915
    ...
    for override in args:
        if "=" not in override:
            raise ValueError(
                f"Invalid configuration override: {override!r}. "
                "Expected KEY=VALUE."
            )

        key, raw_value = override.split("=", 1)

        # Use OmegaConf to preserve value types:
        # "1" -> int, "null" -> None, "true" -> bool, etc.
        value_config = omegaconf.OmegaConf.from_dotlist(
            [f"value={raw_value}"]
        )
        value = omegaconf.OmegaConf.to_container(
            value_config,
            resolve=False,
        )["value"]
