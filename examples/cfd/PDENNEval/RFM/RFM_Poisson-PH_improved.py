...
class Net(nn.Module):
    def __init__(self, hidden_size, dimension):
        ...
        nn.init.xavier_normal_(self.input_layers.weight, gain=nn.init.calculate_gain('tanh'))
        nn.init.normal_(self.input_layers.bias, mean=0.0, std=1 / hidden_size)
...
net = Net(n_basis_func, dimension)
