...
def main():
    ...
    channels_weights = torch.sqrt(torch.mean(y.permute(0, 2, 3, 1).reshape((batch*nx*ny,3)) ** 2, dim=0)).view(1, -1, 1, 1).to(options["device"])
    if dist.rank == 0:
    ...
    if dist.rank == 0:
        checkpoint = {
            "model_state": DeepCFD.module.state_dict() if dist.world_size > 1 else DeepCFD.state_dict(),
            "config": {
