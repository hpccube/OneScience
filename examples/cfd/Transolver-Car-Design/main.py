...
if __name__ == '__main__':
    ...
    parser.add_argument('--preprocessed', default=1, type=int, help='Whether to use preprocessed data')
    ...
    train_data, val_data, coef_norm = load_train_val_fold(args, preprocessed=args.preprocessed, dist=dist)
    ...
    if args.cfd_model == 'Transolver':
        model = Transolver3D(
            n_hidden=256, n_layers=8, space_dim=7,
            fun_dim=0, n_head=8, mlp_ratio=2,
            out_dim=4, slice_num=32, unified_pos=0
        )

    save_path = os.path.join(args.model_save_dir, args.cfd_model, str(args.fold_id), f'{args.nb_epochs}_{args.weight}')
    if dist.rank == 0:
