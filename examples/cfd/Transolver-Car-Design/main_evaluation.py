...
parser.add_argument('--preprocessed_save_dir', default='./dataset/mlcfd_data/preprocessed_data')
parser.add_argument('--model_save_dir', default='./metrics')
parser.add_argument('--result_dir', default='./results', type=str)
parser.add_argument('--fold_id', default=0, type=int, help=' Which param folder should be selected as the test/validation set')
parser.add_argument('--gpu', default=0, type=int)
parser.add_argument('--cfd_model',default='Transolver') 
parser.add_argument('--cfd_mesh', action='store_true')
parser.add_argument('--r', default=0.2, type=float)
parser.add_argument('--weight', default=0.5, type=float, help='Weight loss of pressure term. default=0.5')
parser.add_argument('--nb_epochs', default=200, type=int)
parser.add_argument('--save_vtk', action='store_true', help='Save predictions to VTK files')
...
if args.visualize and not args.save_vtk:
    raise ValueError("--visualize needs to be used with --save_vtk to generate visualization from the predicted VTK files.")
...
path = os.path.join(args.model_save_dir, args.cfd_model, str(args.fold_id), f'{args.nb_epochs}_{args.weight}')
...
with torch.no_grad():
    ...
    for cfd_data, geom in test_loader:
        ...
        if coef_norm is not None: #反归一化
            ...
            pred_press = out[cfd_data.surf, -1] * std[-1] + mean[-1] # 只需要车辆表面上的压力值
            gt_press = targets[cfd_data.surf, -1] * std[-1] + mean[-1] 
            pred_velo = out[~cfd_data.surf, :-1] * std[:-1] + mean[:-1] # 只需要车辆附近的速度值
            gt_velo = targets[~cfd_data.surf, :-1] * std[:-1] + mean[:-1] 
            out_denorm = out * std + mean
            y_denorm = targets * std + mean

        np.save(os.path.join(npy_dir, f'{index}_pred.npy'), out_denorm.detach().cpu().numpy())
        np.save(os.path.join(npy_dir, f'{index}_gt.npy'), y_denorm.detach().cpu().numpy())
        
        data_dir_for_sample = os.path.join(args.data_dir, f'param{args.fold_id}', vallst[index].split('/')[1])
        pred_coef = cal_coefficient(data_dir_for_sample, pred_press[:, None].detach().cpu().numpy(), pred_velo.detach().cpu().numpy())       
        gt_coef = cal_coefficient(data_dir_for_sample, gt_press[:, None].detach().cpu().numpy(), gt_velo.detach().cpu().numpy())

        # 打印当前样本结果
        sample_error = abs(pred_coef - gt_coef) / gt_coef
        print(f"  Ground Truth CD: {gt_coef:.6f}   Predicted CD: {pred_coef:.6f}   Relative Error: {sample_error:.6}\n")
    ...
    print(f"- Spearman correlation (rho_d): {sc.stats.spearmanr(gt_coef_list, pred_coef_list)[0]:.6f}")
