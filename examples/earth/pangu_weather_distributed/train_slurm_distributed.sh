...
srun -u --mpi=pmix \
    bash -c "
    source export_DDP_vars.sh
    python train_distributed_4stage.py --micro-batch-size 1 --global-batch-size 1 --encoder-seq-length=1 --num-layers=4 --hidden-size=256 \
            --num-attention-heads=8 --max-position-embeddings=1 --tokenizer-type=NullTokenizer --vocab-size=128 --lr-decay-style linear \
            --train-iters 219000 --lr 0.0001 --pipeline-model-parallel-size 4 --tensor-model-parallel-size 2 \
            --eval-interval 4380 --eval-iters 438 \
            --init-method-std 0.005 \
            --weight-decay 0.001 --adam-beta2 0.95 \
            --log-interval 1 \
