...
def main():
    ...
    # 提取中国区域数据
    china_predictions = predictions.sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))
    china_eval = eval_targets.sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))
    # 推理结果可视化
