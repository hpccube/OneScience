...
def main():
    ...
    train_data_loader, train_dataset, train_sampler = get_data_loader(params, False, mode='train')
    valid_data_loader, valid_dataset, valid_sampler = get_data_loader(params, False, mode='valid')
    ...
    for epoch in range(params.max_epochs):
        ...
        with torch.no_grad():
            for iteration_idx, data in enumerate(valid_data_loader):
                ...
                if world_rank == 0 and iteration_idx % (len(valid_data_loader) // 3) == 0:
        ...
        if valid_loss < best_valid_loss:
            ...
            if world_rank == 0:
                save_checkpoint(model, optimizer, scheduler, best_valid_loss, best_loss_epoch, model_name)
