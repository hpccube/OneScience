...
wind_u_dir = '/public/SothisAI/sharingCenter/data/easyscience/Ocean/Wind_U10'
wind_v_dir = '/public/SothisAI/sharingCenter/data/easyscience/Ocean/Wind_V10'
...
os.makedirs(f'{Wind_Cos}', exist_ok=True)
os.makedirs(f'{Wind_Sin}', exist_ok=True)
os.makedirs(f'{Wind_Strength}', exist_ok=True)

def process_all_years():
    
    wind_u_files = sorted([f for f in os.listdir(f'{wind_u_dir}') if f.endswith('.h5')])
    wind_u_files = wind_u_files[:-1]
    for year_file in wind_u_files:
        ...
        with h5py.File(wind_u_path, 'r') as u_file, h5py.File(wind_v_path, 'r') as v_file:
