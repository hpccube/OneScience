...
from onescience.utils.fcn.weighted_acc_rmse import weighted_acc, weighted_rmse, weighted_rmse_torch, unlog_tp_torch
import wandb
from onescience.utils.fcn.img_utils import vis_precip
from onescience.models.afno.afnonet import AFNONet, PrecipNet
...
class Trainer():
    ...
    def __init__(self, params, world_rank):
        ...
        if params.log_to_wandb:
            wandb.init(config=params, name=params.name, group=params.group,
                       project=params.project, entity=params.entity)
        ...
        if self.precip:
            ...
            # no backprop through the wind model
        ...
        if params.two_step_training:
            if params.resuming == False and params.pretrained == True:
                logging.info(
                    "Starting from pretrained one-step afno model at %s" % params.pretrained_ckpt_path)
...
#          logging.info('Final Valid RMSE: Z500- {}. T850- {}, 2m_T- {}'.format(valid_weighted_rmse[0], valid_weighted_rmse[1], valid_weighted_rmse[2]))
    ...
    def train_one_epoch(self):
        ...
        for i, data in enumerate(self.train_data_loader, 0):
            ...
            inp, tar = map(lambda x: x.to(
                self.device, dtype=torch.float), data)
        ...
        with torch.no_grad():
            for i, data in enumerate(self.valid_data_loader, 0):
                ...
                if (i == sample_idx) and (self.precip and self.params.log_to_wandb):
                    fields = [gen[0, 0].detach().cpu().numpy(),
                ...
                if self.params.two_step_training:
                    if 'residual_field' in self.params.target:
                        valid_weighted_rmse += weighted_rmse_torch(
                            (gen_step_one + inp), (tar[:, 0:self.params.N_out_channels] + inp))
                    else:
                        valid_weighted_rmse += weighted_rmse_torch(
                            gen_step_one, tar[:, 0:self.params.N_out_channels])
                else:
                ...
                if not self.precip:
                    ...
                    if self.params.two_step_training:
                        save_image(torch.cat((gen_step_one[0, 0], torch.zeros((self.valid_dataset.img_shape_x, 4)).to(
                            self.device, dtype=torch.float), tar[0, 0]), axis=1), params['experiment_dir'] + "/" + str(i) + "/" + str(self.epoch) + ".png")
                    else:
                        save_image(torch.cat((gen[0, 0], torch.zeros((self.valid_dataset.img_shape_x, 4)).to(
                            self.device, dtype=torch.float), tar[0, 0]), axis=1), params['experiment_dir'] + "/" + str(i) + "/" + str(self.epoch) + ".png")
    ...
    def validate_final(self):
        ...
        valid_weighted_rmse = torch.zeros(
            n_valid_batches, self.params.N_out_channels)
        if self.params.normalization == 'minmax':
            raise Exception("minmax normalization not supported")
        elif self.params.normalization == 'zscore':
            mult = torch.as_tensor(np.load(self.params.global_stds_path)[
        ...
        with torch.no_grad():
            ...
            if self.params.two_step_training:
                for c in range(self.params.N_out_channels):
                    if 'residual_field' in self.params.target:
                        valid_weighted_rmse[i, c] = weighted_rmse_torch(
                            (gen_step_one[0, c] + inp[0, c]), (tar[0, c]+inp[0, c]), self.device)
                    else:
                        valid_weighted_rmse[i, c] = weighted_rmse_torch(
                ...
                for c in range(self.params.N_out_channels):
                    if 'residual_field' in self.params.target:
                        valid_weighted_rmse[i, c] = weighted_rmse_torch(
                            (gen[0, c] + inp[0, c]), (tar[0, c]+inp[0, c]), self.device)
                    else:
                        valid_weighted_rmse[i, c] = weighted_rmse_torch(
            ...
            # un-normalize
            valid_weighted_rmse = mult * \
                torch.mean(valid_weighted_rmse[0:100], axis=0).to(self.device)
    ...
    def load_model_wind(self, model_path):
        if self.params.log_to_screen:
            logging.info(
                'Loading the wind model weights from {}'.format(model_path))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--yaml_config", default='./conf/AFNO.yaml', type=str)
