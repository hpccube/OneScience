...
from onescience.utils.fcn.weighted_acc_rmse import (
...
from onescience.models.afno import AFNONet
import glob
...
def setup(params):
    ...
    if params.log_to_screen:
        logging.info('Loading trained model checkpoint from {}'.format(
...
def autoregressive_inference(params, ic, valid_data_full, model):
    ...
    valid_loss_coarse = torch.zeros(
        (prediction_length, n_out_channels)).to(device, dtype=torch.float)
    acc_coarse = torch.zeros((prediction_length, n_out_channels)).to(
        device, dtype=torch.float)
    acc_coarse_unweighted = torch.zeros(
        (prediction_length, n_out_channels)).to(device, dtype=torch.float)

    acc_unweighted = torch.zeros((prediction_length, n_out_channels)).to(
        device, dtype=torch.float)
    seq_real = torch.zeros((prediction_length, n_in_channels,
                           img_shape_x, img_shape_y)).to(device, dtype=torch.float)
    ...
    if params.masked_acc:
        maskarray = torch.as_tensor(np.load(params.maskpath)[
                                    0:720]).to(device, dtype=torch.float)

    # extract valid data from first year
    valid_data = valid_data_full[ic:(
        ic+prediction_length*dt+n_history*dt):dt, in_channels, 0:720]
    # standardize
    ...
    if not params.use_daily_climatology:
        m = torch.as_tensor((np.load(params.time_means_path)[
                            0][out_channels] - means)/stds)[:, 0:img_shape_x]  # climatology
        ...
        with h5py.File(dc_path, 'r') as f:
            dc = f['time_means_daily'][ic:ic +
                                       prediction_length*dt:dt]  # 1460,21,721,1440
        m = torch.as_tensor(
    ...
    if orography:
        orog = torch.as_tensor(np.expand_dims(np.expand_dims(h5py.File(orography_path, 'r')[
                               'orog'][0:720], axis=0), axis=0)).to(device, dtype=torch.float)
        ...
        for i in range(valid_data.shape[0]):
            if i == 0:  # start of sequence
                ...
                for h in range(n_history+1):
                    ...
                    seq_real[h] = first[h *
                                        n_in_channels: (h+1)*n_in_channels][0:n_out_channels]
                    seq_pred[h] = seq_real[h]
                if params.perturb:
                    first = gaussian_perturb(
                        first, level=params.n_level, device=device)  # perturb the ic
                ...
                if orography:
                    ...
                    logging.info(f"first: { future_pred.shape }")
            ...
            acc_unweighted[i] = unweighted_acc_torch_channels(
                pred-clim, tar-clim)
            ...
            if params.interp > 0:
                ...
                valid_loss_coarse[i] = weighted_rmse_torch_channels(
                    pred, tar) * std
            ...
            if params.log_to_screen:
                idx = idxes[fld]
                logging.info('Predicted timestep {} of {}. {} RMS Error: {}, ACC: {}'.format(
    ...
    print(
        f"duras: {valid_data.shape[0]}, { duras }, { sum(duras)/ len(duras) } s")
    ...
    return (np.expand_dims(seq_real[n_history:], 0), np.expand_dims(seq_pred[n_history:], 0), np.expand_dims(valid_loss, 0), np.expand_dims(acc, 0),
            np.expand_dims(acc_unweighted, 0), np.expand_dims(
                valid_loss_coarse, 0), np.expand_dims(acc_coarse, 0),
...
if __name__ == '__main__':
    ...
    params['best_checkpoint_path'] = args.weights if args.override_dir is not None else os.path.join(
    ...
    for i, ic in enumerate(ics):
        ...
        if i == 0 or len(valid_loss) == 0:
            ...
            acc_coarse_unweighted = np.concatenate(
                (acc_coarse_unweighted, acu), 0)
    ...
    if params.log_to_screen:
        logging.info("Saving files at {}".format(os.path.join(
            params['experiment_dir'], 'autoregressive_predictions' + autoregressive_inference_filetag + '.h5')))
    with h5py.File(os.path.join(params['experiment_dir'], 'autoregressive_predictions' + autoregressive_inference_filetag + '.h5'), 'a') as f:
        if vis:
            try:
                f.create_dataset("ground_truth", data=seq_real, shape=(
                    n_ics, prediction_length, n_out_channels, img_shape_x, img_shape_y), dtype=np.float32)
            except:
                del f["ground_truth"]
                f.create_dataset("ground_truth", data=seq_real, shape=(
                    n_ics, prediction_length, n_out_channels, img_shape_x, img_shape_y), dtype=np.float32)
            ...
            try:
                f.create_dataset("predicted", data=seq_pred, shape=(
                    n_ics, prediction_length, n_out_channels, img_shape_x, img_shape_y), dtype=np.float32)
            except:
                del f["predicted"]
                f.create_dataset("predicted", data=seq_pred, shape=(
        ...
        if params.masked_acc:
            try:
                ...
                f.create_dataset("acc_land", data=acc_land)
            except:
                del f["acc_land"]
                # , shape = (n_ics, prediction_length, n_out_channels), dtype =np.float32)
                f.create_dataset("acc_land", data=acc_land)
            ...
            try:
                # , shape = (n_ics, prediction_length, n_out_channels), dtype =np.float32)
                f.create_dataset("acc_sea", data=acc_sea)
            except:
                del f["acc_sea"]
                # , shape = (n_ics, prediction_length, n_out_channels), dtype =np.float32)
                f.create_dataset("acc_sea", data=acc_sea)
        ...
        try:
            f.create_dataset("rmse", data=valid_loss, shape=(
                n_ics, prediction_length, n_out_channels), dtype=np.float32)
        except:
            del f["rmse"]
            f.create_dataset("rmse", data=valid_loss, shape=(
                n_ics, prediction_length, n_out_channels), dtype=np.float32)
        ...
        try:
            f.create_dataset("acc", data=acc, shape=(
                n_ics, prediction_length, n_out_channels), dtype=np.float32)
        except:
            del f["acc"]
            f.create_dataset("acc", data=acc, shape=(
        ...
        try:
            f.create_dataset("rmse_coarse", data=valid_loss_coarse, shape=(
                n_ics, prediction_length, n_out_channels), dtype=np.float32)
        except:
            del f["rmse_coarse"]
            f.create_dataset("rmse_coarse", data=valid_loss_coarse, shape=(
        ...
        try:
            f.create_dataset("acc_coarse", data=acc_coarse, shape=(
                n_ics, prediction_length, n_out_channels), dtype=np.float32)
        except:
            del f["acc_coarse"]
            f.create_dataset("acc_coarse", data=acc_coarse, shape=(
        ...
        try:
            f.create_dataset("acc_unweighted", data=acc_unweighted, shape=(
                n_ics, prediction_length, n_out_channels), dtype=np.float32)
        except:
        ...
        try:
            f.create_dataset("acc_coarse_unweighted", data=acc_coarse_unweighted, shape=(
                n_ics, prediction_length, n_out_channels), dtype=np.float32)
        except:
