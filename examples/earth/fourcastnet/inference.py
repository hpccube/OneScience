...
def autoregressive_inference(params, ic, valid_data_full, model):
    ...
    print(f"duras: {valid_data.shape[0]}, { duras }, { sum(duras)/ len(duras) } s")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights", default=None, type=str, help="Path to model weights"
    )
    parser.add_argument("--output_dir", default="./output", help="Path to save output.")
    ...
    if not os.path.isdir(args.output_dir):
        logging.warning(
            f"args.output_dir: {args.output_dir} is not exists and try to create..."
        )
