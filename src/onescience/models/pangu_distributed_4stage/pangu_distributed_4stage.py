...
class Pangu_stage3(Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(1, 2).reshape(B, -1, Pl, Lat, Lon)

        output_surface = x[:, :, 0, :, :]
        output_upper_air = x[:, :, 1:, :, :]
