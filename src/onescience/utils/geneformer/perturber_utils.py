...
def _load_trusted_local_bin(
    model_class,
    model_directory,
    output_hidden_states,
    num_labels=None,
):
    """Load a trusted local legacy checkpoint without Transformers' torch.load path.

    Transformers 4.56 refuses all ``pytorch_model.bin`` files with torch < 2.6.
    OneScience's DTK torch build is intentionally kept at 2.5, so local Geneformer
    checkpoints supplied by the user are loaded with PyTorch's restricted
    ``weights_only=True`` unpickler instead. Safetensors and remote checkpoints
    continue to use ``from_pretrained``.
    """
    model_path = Path(model_directory)
    checkpoint_path = model_path / "pytorch_model.bin"
    if not model_path.is_dir() or not checkpoint_path.is_file():
        raise FileNotFoundError(
            "The legacy Geneformer compatibility loader only supports a local "
    ...
    if num_labels is not None:
        config_args["num_labels"] = num_labels
    ...
    unexpected_keys = [
        key for key in incompatible.unexpected_keys if not key.endswith("position_ids")
    ]
    if incompatible.missing_keys or unexpected_keys:
        logger.warning(
            "Legacy checkpoint loaded with missing keys %s and unexpected keys %s.",
...
def load_model(model_type, num_classes, model_directory, mode, quantize=False):
    ...
    if quantize:
        try:
            ...
            raise RuntimeError(
                "Geneformer quantization requires a working PEFT installation and "
                "compatible accelerator runtime. Full-precision inference and "
                "fine-tuning do not import PEFT."
            ) from exc
    ...
    # Load the model. The narrow fallback keeps the site-provided DTK torch build
    # intact while supporting trusted, local V1 checkpoints saved as PyTorch bins.
    try:
        model = model_class.from_pretrained(**model_args)
    except ValueError as exc:
        legacy_torch_block = "upgrade torch to at least v2.6" in str(exc)
        local_bin = Path(model_directory, "pytorch_model.bin").is_file()
        local_safetensors = any(Path(model_directory).glob("*.safetensors"))
        if quantize_config or not legacy_torch_block or not local_bin or local_safetensors:
            raise
        model = _load_trusted_local_bin(
            model_class,
            model_directory,
            output_hidden_states,
            num_labels=None if model_type == "Pretrained" else num_classes,
        )
