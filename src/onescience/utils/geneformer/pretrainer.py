...
class GeneformerPreCollator(SpecialTokensMixin):
    ...
    def save_pretrained(self, save_directory, **kwargs):
        """Persist the vocabulary when newer Transformers saves a checkpoint."""
        save_directory = Path(save_directory)
        save_directory.mkdir(parents=True, exist_ok=True)
        vocabulary_file = save_directory / "token_dictionary.pkl"
        with vocabulary_file.open("wb") as handle:
            pickle.dump(self.token_dictionary, handle)
        return (str(vocabulary_file),)
...
class GeneformerPretrainer(Trainer):
    ...
    def _get_train_sampler(
        ...
        # Transformers >= 4.56 passes the prepared dataset to this hook, while
        # older releases call it without arguments.
        active_dataset = (
            train_dataset if train_dataset is not None else self.train_dataset
        )
        if not isinstance(active_dataset, collections.abc.Sized):
            return None
        ...
        if self.args.group_by_length:
            if is_datasets_available() and isinstance(active_dataset, Dataset):
            ...
            return RandomSampler(active_dataset)
