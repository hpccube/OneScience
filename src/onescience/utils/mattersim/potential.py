...
from onescience.models.mattersim.mattersim import M3Gnet
...
def batch_to_dict(graph_batch, model_type="m3gnet", device="cuda"):
    if model_type == "m3gnet":
        ...
        num_three_body_cpu = graph_batch.num_three_body.cpu()
        num_bonds_cpu = graph_batch.num_bonds.cpu()
        num_triple_ij_cpu = graph_batch.num_triple_ij.cpu()
        num_three_body = num_three_body_cpu.to(device)
        num_bonds = num_bonds_cpu.to(device)
        num_triple_ij = num_triple_ij_cpu.to(device)
        ...
        cumsum = torch.cumsum(num_bonds_cpu, dim=0) - num_bonds_cpu
        input["bond_index_bias"] = torch.repeat_interleave(
            cumsum, num_three_body_cpu, dim=0
        ...
        input["three_body_edge_map"] = torch.repeat_interleave(
            index_map, num_triple_ij_cpu.view(-1)
        ).to(device)
