...
from onescience.utils.nequip.internal import get_current_code_versions, RankedLogger
from onescience.utils.nequip.internal.compat import rewrite_nequip_targets
from onescience.utils.nequip.internal.global_state import set_global_state, get_latest_global_state
from onescience.datapipes.materials.nequip.datamodule import NequIPDataModule
from onescience.utils.nequip.train import NequIPLightningModule
...
def _run_with_suppressed_leafspec_warning(trainer_call: Callable, **kwargs) -> None:
    ...
    try:
        parameters = inspect.signature(trainer_call).parameters.values()
        accepts_weights_only = any(
            parameter.name == "weights_only"
            or parameter.kind == inspect.Parameter.VAR_KEYWORD
            for parameter in parameters
        )
    except (TypeError, ValueError):
        # Preserve the current behavior for callables whose signatures cannot be inspected.
        accepts_weights_only = True
...
def _compatible_checkpoint_hyper_parameters(checkpoint: dict) -> dict:
    """Return checkpoint hyperparameters with legacy NequIP targets rewritten."""
    return rewrite_nequip_targets(checkpoint["hyper_parameters"])
...
def main(config: DictConfig) -> None:
    ...
    import onescience.utils.nequip.internal.xpu  # noqa: F401
    ...
    if "ckpt_path" in config:
        ...
        checkpoint_hyper_parameters = _compatible_checkpoint_hyper_parameters(
            checkpoint
        )

        # check that runs from checkpoint match runs from config
        ckpt_runs = checkpoint_hyper_parameters["info_dict"]["runs"]
        ...
        training_module = hydra.utils.get_class(
            checkpoint_hyper_parameters["info_dict"]["training_module"]["_target_"]
        )
        # propagate info dict
        info_dict.update(
            {
                "training_module": checkpoint_hyper_parameters["info_dict"][
                    "training_module"
                ]
            }
        )
        checkpoint_hyper_parameters["info_dict"] = info_dict
        checkpoint_hyper_parameters["num_datasets"] = datamodule.num_datasets
