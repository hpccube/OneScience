...
from onescience.models.nequip.__init__ import _DISCOVERED_NEQUIP_EXTENSION


_E3NN_CAN_BE_INTERNED = Version(e3nn.__version__) >= Version("0.5")

_INTERNAL_MODULES: Final[Set[str]] = set(
    (["e3nn"] if _E3NN_CAN_BE_INTERNED else [])
    + [
        "onescience.models.nequip",
        "onescience.datapipes.materials.nequip",
        "onescience.utils.nequip",
    ]
...
_DEFAULT_EXTERNAL_MODULES: Final[Set[str]] = {
    ...
    "packaging",
    # imported by e3nn 0.4.x spherical harmonics code generation
    "sympy",
}
_EXTERNAL_MODULES: Set[str] = set(_DEFAULT_EXTERNAL_MODULES)
if not _E3NN_CAN_BE_INTERNED:
    # e3nn 0.4.x reads o3/constants.pt through its real filesystem path.
    # torch.package uses virtual module paths, so this version must remain external.
    _EXTERNAL_MODULES.add("e3nn")
...
def _nequip_packaging_mode() -> List[str]:
    from onescience.models.nequip.model.utils import _EAGER_MODEL_KEY, _COMPILE_MODE_OPTIONS
    from onescience.utils.nequip.internal.versions import _TORCH_GE_2_6
