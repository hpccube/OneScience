...
def set_global_state(
    ...
    if not _GLOBAL_STATE_INITIALIZED:
        ...
        # NOTE: patched for OneScience to support both e3nn 0.4.x and 0.6.x;
        # the ``jit_mode`` option is only available in e3nn >= 0.5.0.
        _e3nn_opt_kwargs = dict(
            specialized_code=True,
            optimize_einsums=True,
        )
        import inspect
        if "jit_mode" in inspect.signature(e3nn.set_optimization_defaults).parameters:
            _e3nn_opt_kwargs["jit_mode"] = "eager" if _TORCH_GE_2_10 else "script"
        e3nn.set_optimization_defaults(**_e3nn_opt_kwargs)
