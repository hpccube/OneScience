...
from onescience.utils.nequip.internal import get_project_root
...
def register_resolvers(resolvers: Dict[str, Callable]) -> None:
    ...
    """
    # Register each resolver with OmegaConf, skipping names that are already
    # registered globally. This allows NequIP to coexist with other OneScience
    # packages that may register resolvers with the same name.
    for name, func in resolvers.items():
        if OmegaConf.has_resolver(name):
            logger.debug("OmegaConf resolver '%s' is already registered; skipping.", name)
            continue
