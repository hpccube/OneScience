...
from onescience.modules.layer.uma_layer_norm import get_normalization_layer
...
class Rank2SymmetricTensorHead(nn.Module, HeadInterface):
    ...
    def forward(
        ...
        if self.use_source_target_embedding:
            x_source = node_emb[emb["edge_index"][0]]
            x_target = node_emb[emb["edge_index"][1]]
            x_edge = torch.cat((x_source, x_target), dim=2)
        else:
            x_edge = node_emb[emb["edge_index"][1]]
        ...
        if self.decompose:
            tensor_0, tensor_2 = self.block(
                emb["edge_distance_vec"], x_edge, emb["edge_index"][1], data
            )
