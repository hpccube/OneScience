...
from onescience.distributed.megatron.core.transformer.moe.auto_dim_moe_layer import AutoDimMoELayer, DynamicDimMoE
...
class EarthDistributedTransformer3DBlock(nn.Module):
    ...
    def __init__(
        ...
        #     config = config
        # )
        self.num_experts = getattr(config, 'num_moe_experts', None)
        ...
        if self.num_experts is None:
            ...
            self.mlp = AutoDimMoELayer(
                config=config,
                submodules=moe_spec.submodules,
