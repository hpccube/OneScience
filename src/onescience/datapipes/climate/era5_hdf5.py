...
#         self.start_year = 2020 # int(self.data_paths[0][:-3])
...
#
#         timestamps = datetime(self.start_year + outvar_year_idx, 1, 1, tzinfo=pytz.utc) + timedelta(hours=outvar_in_idx * self.dt)
#         timestamps = torch.tensor([timestamps.timestamp()])
#         cos_zenith = cos_zenith_angle(timestamps, latlon=self.latlon_torch).float()
#
#         return invar.squeeze(0), outvar.squeeze(0), cos_zenith, invar_in_idx
#
#     def __len__(self):
