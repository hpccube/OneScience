...
def unstructured_grid_data_to_poly_data(unstructured_grid_data): #将非结构化网格转为表面多边形数据
...
def visualize_poly_data(poly_data, surface_filter, scalar_data=None, colorbar_title="Data", save_path=None):
    # 创建渲染器和窗口（不显示）
    ...
    if scalar_data is not None:
        ...
        mapper.SetScalarRange(np.min(scalar_data), np.max(scalar_data))  # 使用传入数据的范围
    else:
        # 如果没有传入 scalar_data，尝试从 poly_data 中提取标量值
        if poly_data.GetPointData().GetScalars() is not None:
            mapper.SetLookupTable(lut)  # 设置颜色映射表
            mapper.SetScalarModeToUsePointData()
            scalar_range = poly_data.GetPointData().GetScalars().GetRange()  # 从数据中获取标量范围
...
def get_cardatalist(root, samples, norm=False, coef_norm=None, savedir=None, preprocessed=False):
    dataset = []
...
def save_prediction_to_vtk(out_denorm, targets, cfd_data, sample_name, output_dir, index, data_dir):
    """将预测结果保存为VTK文件"""
    # 解析样本完整路径
    ...
    surface_points = set(tuple(p) for p in vtk_to_numpy(press_grid.GetPoints().GetData()))
    exterior_indices = [i for i, p in enumerate(points_velo) if tuple(p) not in surface_points]
...
def visualize_prediction(output_dir, vis_dir, index):
    ...
    try:
        ...
        # 可视化压力预测
        press_poly, press_filter = unstructured_grid_data_to_poly_data(pred_press)
        visualize_poly_data(
            press_poly,
            press_filter,
        ...
        # 可视化速度预测
        velo_poly, velo_filter = unstructured_grid_data_to_poly_data(pred_velo)
        speed_data = get_speed_from_poly_data(velo_poly)
        visualize_poly_data(
            velo_poly,
            velo_filter,
            scalar_data=speed_data,
            colorbar_title="Predicted Speed",
...
def get_shape(data, max_n_point=8192, normalize=True, use_height=False): #提取表面形状特征
...
def get_airfoildatalist(set, norm=False, coef_norm=None, crop=None, sample=None, n_boot=int(5e5), surf_ratio=.1,
            data_path='/data/path'):
    '''
...
if __name__ == '__main__':
    ...
    root = '/public/home/songzhl/bao/Transolver-main/Car-Design-ShapeNetCar/dataset/mlcfd_data/training_data'
    save_path = '/public/home/songzhl/bao/Transolver-main/Car-Design-ShapeNetCar/dataset/mlcfd_data/preprocessed_data/param0/' + file_name
    ...
    # 示例：可视化压力网格表面并保存
    poly_data, surface_filter = unstructured_grid_data_to_poly_data(unstructured_grid_data_press)
    ...
    # 调用示例
    poly_data_velo, surface_filter_velo = unstructured_grid_data_to_poly_data(unstructured_grid_data_velo)
    # 从表面网格数据获取速度
