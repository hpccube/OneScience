...
from onescience.datapipes.boltz.pad import pad_dim
from onescience.utils.boltz.geometry import lddt_dist, weighted_rigid_align
...
def weighted_minimum_rmsd_single(
    ...
    """Align coordinates and compute the weighted RMSD for one structure."""
    ...
    squared_error = ((pred_atom_coords - aligned_coords) ** 2).sum(dim=-1)
    rmsd = torch.sqrt(
        torch.sum(squared_error * align_weights * atom_mask, dim=-1)
        / torch.sum(align_weights * atom_mask, dim=-1)
    )
    return rmsd, aligned_coords, align_weights
