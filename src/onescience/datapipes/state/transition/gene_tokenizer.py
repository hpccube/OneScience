...
def get_default_gene_vocab(vocab_file: Union[Path, str, None] = None) -> GeneVocab:
    """
    Get the default gene vocabulary, consisting of gene symbols and ids.
    """
    vocab_file = Path(
        vocab_file
        or os.environ.get("STATE_GENE_VOCAB", "")
        or Path(__file__).parent / "default_gene_vocab.json"
    ).expanduser()
    if not vocab_file.exists():
        raise FileNotFoundError(
            f"Default gene vocabulary not found at {vocab_file}. "
            "Provide this local asset before calling get_default_gene_vocab()."
        )
...
def _build_default_gene_vocab(
    ...
    if not gene_collection_file.exists():
        raise FileNotFoundError(
            f"HGNC gene collection not found at {gene_collection_file}. "
            "Download it outside OneScience and pass its containing directory via download_source_to."
        )
